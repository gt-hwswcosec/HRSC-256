-- Parallelized version of HRSC-256 (8 keystream bits/clock cycle)
library ieee;
...
entity hybrid is
  port(
    ...
    --   8 keystream bits per clock while in RUN
    keystream_bits_out : out std_logic_vector(7 downto 0)
  );
end hybrid;
...
architecture encrypt of hybrid is
  ...
  -- setup counter counts SETUP CLOCKS (0..15). Each setup clock performs 2 state updates.
  signal setup_clk_count : integer range 0 to 16 := 0;
...
begin
  ...
  -----------------------------------------------------------------------------
  -- Parallelized 8-bit/cycle engine
  -----------------------------------------------------------------------------
  process(clk, rst)
    variable s1, s2, s3, s4, s5, s6, s7, s8 : std_logic_vector(511 downto 0);
    variable y0, y1, y2, y3, y4, y5, y6, y7 : std_logic;
  begin
    if rst = '1' then
      -- init state
      state        <= load_key;
      ...
      case state is
        ...
        when setup =>
          o_vld <= '0';
          keystream_bits_out <= (others => '0');
		  -- compute 8-step unroll from the current registered state
		  s1 := nxt(s_reg);  -- state after 1 update
		  s2 := nxt(s1);     -- state after 2 updates
		  s3 := nxt(s2);     -- state after 3 updates
		  s4 := nxt(s3);     -- state after 4 updates
		  s5 := nxt(s4);     -- state after 5 updates
		  s6 := nxt(s5);     -- state after 6 updates
		  s7 := nxt(s6);     -- state after 7 updates
		  s8 := nxt(s7);     -- state after 8 updates
          -- perform 8 updates per setup clock
          s_reg <= s8;

          -- count SETUP CLOCKS, not steps: 16 clocks = 128 updates
          if setup_clk_count = 15 then
            state <= run;
            setup_clk_count <= 0;
          else
        ...
        when run =>
          ...
          -- output eight consecutive keystream bits based on state and next-state
          y0 := ks(s_reg); -- bit at time t
          y1 := ks(s1);    -- bit at time t+1
		  y2 := ks(s2);    -- bit at time t+2
		  y3 := ks(s3);    -- bit at time t+3
		  y4 := ks(s4);    -- bit at time t+4
		  y5 := ks(s5);    -- bit at time t+5
		  y6 := ks(s6);    -- bit at time t+6
		  y7 := ks(s7);    -- bit at time t+7

          o_vld <= '1';
          keystream_bits_out(0) <= y0;
          keystream_bits_out(1) <= y1;
		  keystream_bits_out(2) <= y2;
		  keystream_bits_out(3) <= y3;
		  keystream_bits_out(4) <= y4;
		  keystream_bits_out(5) <= y5;
		  keystream_bits_out(6) <= y6;
		  keystream_bits_out(7) <= y7;

          -- advance 8 steps per clock
          s_reg <= s8;
      end case;
    end if;
  end process;
