-- Parallelized version of HRSC-256 (4 keystream bits/clock cycle)
library ieee;
...
entity hybrid is
  port(
    ...
    --   4 keystream bits per clock while in RUN
    keystream_bits_out : out std_logic_vector(3 downto 0)
  );
end hybrid;
...
architecture encrypt of hybrid is
  ...
  -- setup counter counts SETUP CLOCKS (0..31). Each setup clock performs 2 state updates.
  signal setup_clk_count : integer range 0 to 32 := 0;
...
begin
  ...
  -----------------------------------------------------------------------------
  -- Parallelized 4-bit/cycle engine
  -----------------------------------------------------------------------------
  process(clk, rst)
    variable s1, s2, s3, s4 : std_logic_vector(511 downto 0);
    variable y0, y1, y2, y3 : std_logic;
  begin
    if rst = '1' then
      -- init state
      state        <= load_key;
      ...
      case state is
        ...
        when setup =>
          o_vld <= '0';
          keystream_bits_out <= (others => '0');
		  -- compute 4-step unroll from the current registered state
          ...
          -- perform 4 updates per setup clock
          s_reg <= s4;

          -- count SETUP CLOCKS, not steps: 32 clocks = 128 updates
          if setup_clk_count = 31 then
            state <= run;
            setup_clk_count <= 0;
          else
        ...
        when run =>
          ...
          -- output four consecutive keystream bits based on state and next-state
